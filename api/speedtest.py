...
def _speedtestable_subscription_node_ids(subscription_id: str, nodes: list) -> list[str]:
    ...
    return [
        node_id
        for node_id, node in zip(node_ids, materialized_nodes, strict=False)
...
async def _speedtest_single(node_id: str, test_speed: bool = False, timeout: int = 10):
    ...
    return {"node_id": node_id, "name": node.get("name", "Unknown"), "result": result}
...
async def _speedtest_batch(node_ids: List[str], test_speed: bool = False, timeout: int = 10, concurrency: int = 10):
    ...
    results = [
        {"node_id": node_ids[idx], "error": str(result)} if isinstance(result, BaseException) else result
...
def get_speedtest_config(_: bool = Depends(verify_session)):
    ...
    get_speedtest_service()
    return {"service_url": AppConfig.GO_SPEEDTEST_URL, "default_timeout": 10, "default_concurrency": 10}
...
async def run_speedtest_profile(profile_id: str, request: Request, _: bool = Depends(verify_session)):
    ...
    if not subscription_ids:
        subscription_ids = [sub.get("id") for sub in load_config().get("subscriptions", []) if sub.get("enabled", True)]
    results = []
    source_node_ids = {}
    ...
    def persist_profile_results(config: dict):
        ...
        for item in results:
            ...
            if node_id:
                ...
                if source_id:
                    stored_results.setdefault(source_id, {})[node_id] = item.get("result") or {
