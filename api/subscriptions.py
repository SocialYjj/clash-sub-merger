...
from services.node_visibility import apply_node_visibility_to_yaml_content, clear_user_subscription_caches
...
from services.subscription_parser import InvalidContentError, parse_local_subscription
    ...
    *,
    ...
    for subscription in config.get("subscriptions", []):
        ...
        if str(subscription.get("name") or "").strip().casefold() == normalized_name:
            raise HTTPException(status_code=409, detail="A subscription with this name already exists") from None
        if url and str(subscription.get("url") or "") == url:
            raise HTTPException(status_code=409, detail="This subscription URL already exists") from None
    ...
    try:
        async with reject_concurrent_refresh(subscription_id):
            ...
            if latest_subscription.get("type") == "local":
                raise HTTPException(status_code=400, detail="Local subscriptions cannot be refreshed") from None
            ...
            if valid_node_count <= 0:
                raise FetchError("Subscription contains no valid proxy nodes") from None
...
class AddSubscription(BaseModel):
    ...
    def validate_name(cls, v):
        ...
        if "/" in normalized_name or "\\" in normalized_name or ".." in normalized_name:
            raise ValueError("Name contains invalid characters") from None
...
def _validate_local_subscription_content(value: Optional[str]) -> Optional[str]:
    if value is None:
        return None
    if not value.strip():
        raise ValueError("Content cannot be empty") from None
    if len(value.encode("utf-8")) > AppConfig.SUBSCRIPTION_MAX_BYTES:
        raise ValueError("Subscription content exceeds the configured size limit") from None
    return value
...
def list_subscriptions(_: bool = Depends(verify_session)):
    ...
    for subscription in config.get("subscriptions", []):
        ...
        try:
            yaml_data = load_subscription_yaml(subscription["id"], AppConfig.YAML_SOURCE_DIR, use_cache=True)
...
async def add_subscription(data: AddSubscription, _: bool = Depends(verify_session)):
    ...
    try:
        ...
        content, sub_info, node_count = await fetcher.fetch(subscription_url, user_agent=user_agent)
        ...
        if visibility_inherited:
            logger.info(
                "Inherited disabled state for %s node(s) while adding subscription %s", visibility_inherited, sub_id
            )
        ...
        raise HTTPException(status_code=400, detail=f"Failed to fetch subscription: {str(e)}") from e
...
def add_local_subscription(data: AddLocalSubscription, _: bool = Depends(verify_session)):
    ...
    try:
        ...
        if visibility_inherited:
            logger.info(
                "Inherited disabled state for %s node(s) while adding local subscription %s",
                visibility_inherited,
                sub_id,
            )
...
def update_local_subscription(sub_id: str, data: UpdateLocalSubscription, _: bool = Depends(verify_session)):
    ...
    with subscription_write_slot(sub_id):
        ...
        if sub.get("type") != "local":
            raise HTTPException(status_code=400, detail="Not a local subscription") from None
        ...
        updated_nodes = (
            subscription_nodes_from_yaml_content(yaml_content) if yaml_content is not None else existing_nodes
        )
...
async def delete_subscription(sub_id: str, _: bool = Depends(verify_session)):
    ...
    if sub_id == "custom_nodes":
        raise HTTPException(status_code=400, detail="custom_nodes is not a subscription") from None
...
async def update_subscription(sub_id: str, data: UpdateSubscription, _: bool = Depends(verify_session)):
    ...
    if current_subscription.get("type") == "local":
        raise HTTPException(status_code=400, detail="Use the local subscription endpoint") from None
...
async def refresh_all_subscriptions(request: Request, _: bool = Depends(verify_session)):
    ...
    refresh_results = await asyncio.gather(*(refresh_one_subscription(subscription) for subscription in url_subs))
