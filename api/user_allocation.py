    ...
    *,
    ...
    async def get_available_nodes_for_users(_: bool = Depends(verify_session)):
        ...
        if custom_nodes:
            ...
            if available_nodes:
                sources["custom_nodes"] = {"name": "自建节点", "nodes": available_nodes}
        ...
        for sub in config.get("subscriptions", []):
            ...
            try:
                ...
                if available_nodes:
                    sources[sub["id"]] = {"name": sub["name"], "nodes": available_nodes}
        ...
        chain_nodes = [
            reference for reference in chain_references if reference.enabled and reference.source_id == "chain_nodes"
        ]
        chain_pools = [
            reference for reference in chain_references if reference.enabled and reference.source_id == "chain_pools"
        ]
        if chain_nodes:
            sources["chain_nodes"] = {
                "name": "链式代理单节点",
                "nodes": [{"id": reference.stable_id, "name": reference.name} for reference in chain_nodes],
            }
        if chain_pools:
            sources["chain_pools"] = {
                "name": "链式代理池",
                "nodes": [{"id": reference.stable_id, "name": reference.name} for reference in chain_pools],
            }

        node_pool_references = [
            reference for reference in list_node_pool_virtual_references(config) if reference.enabled
        ]
