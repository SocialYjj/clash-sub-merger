...
def get_stats_overview(_: bool = Depends(verify_session)):
    ...
    overview = {
        "subscriptions": {"total": len(subs), "active": len(enabled_subs)},
        "nodes": {
        ...
        "users": {"total": len(users), "active": len(enabled_users)},
        "templates": {"total": len(templates)},
        "admin_tokens": {"total": len(admin_tokens), "enabled": len(enabled_tokens)},
        "best_node": best_node,
    }
...
def get_all_nodes_by_country(_: bool = Depends(verify_session)):
    ...
    return {"countries": sorted_countries, "total": sum(c["count"] for c in sorted_countries)}
