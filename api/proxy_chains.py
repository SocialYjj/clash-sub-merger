...
from helpers import generate_timestamp_id, handle_api_errors, load_subscription_yaml
...
def _validate_proxy_chain_references(rows: List[ProxyChainRow], config: Optional[dict] = None) -> None:
    reference_counts = Counter((node.get("sub_id"), node.get("node_id")) for node in _get_all_nodes_for_chain(config))
    for row in rows:
        for chain_node in row.nodes:
            ...
            if chain_node.type == "node" and chain_node.sub_id == VPNGATE_GROUP_SOURCE:
                if not get_vpngate_node(
                    ...
                    raise HTTPException(
                        status_code=400, detail="Proxy chain contains a missing or disabled VPN Gate node"
                    )
                continue
...
def _serialize_chain_node(node: ProxyChainNode, existing_node: dict | None = None) -> dict:
    ...
    if node.type == "group":
        ...
        if node.group_source != VPNGATE_GROUP_SOURCE:
            stored_node["group_nodes"] = [_serialize_chain_node(member) for member in node.group_nodes or []]
    return stored_node
...
def _serialize_chain_rows(
    ...
    existing_fingerprints = [
        row_fingerprint(row.get("nodes", [])) if isinstance(row, dict) else "" for row in existing_rows
    ]
    for row_index, row in enumerate(rows):
        ...
        matching_index = next(
            (
                index
                for index, fingerprint in enumerate(existing_fingerprints)
...
def update_proxy_chain(chain_id: str, data: UpdateProxyChain, _: bool = Depends(verify_session)):
    ...
    def apply_proxy_chain_update(config: dict) -> dict:
        ...
        if data.name is not None and any(
            chain.get("id") != chain_id and str(chain.get("name") or "").strip().casefold() == data.name.casefold()
            for chain in config.get("proxy_chains", [])
        ):
            raise HTTPException(status_code=409, detail="Proxy chain name already exists")
        for chain in config.get("proxy_chains", []):
