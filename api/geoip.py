...
async def batch_lookup_ips(data: BatchGeoIPRequest, request: Request, _: bool = Depends(verify_session)):
    ...
    try:
        ...
        raise
    for ip, res in zip(ips, task_results, strict=False):
...
def export_geoip_cache_entries(
    ...
    headers = {"Content-Disposition": "attachment; filename=geoip-cache.csv"}
...
def _find_persisted_custom_api(config: dict, api_id: str) -> dict | None:
    return next(
        (
            api
            for api in config.get("geoip_config", {}).get("custom_apis", [])
...
def _enabled_geoip_api_ids(geoip_config: dict) -> list[str]:
    ...
    enabled_ids = [
        api_id for api_id in ("ip-api.com", "ipwhois", "ipinfo") if api_settings.get(api_id, {}).get("enabled", True)
    ]
    enabled_ids.extend(
        api.get("id")
...
async def test_geoip_api(
    ...
    try:
        ...
        if result:
            return {"success": True, "test_ip": test_ip, "result": result}
        else:
            return {"success": False, "test_ip": test_ip, "error": "No result returned"}
    except HTTPException:
        raise
    except Exception as exc:
        logger.warning("GeoIP API test failed for %s: %s", api_id, type(exc).__name__)
        return {"success": False, "test_ip": test_ip, "error": "GeoIP API test failed"}
...
async def test_custom_api(data: TestCustomApiRequest, request: Request, _: bool = Depends(verify_session)):
    ...
    try:
        ...
        if result:
            return {"success": True, "test_ip": data.test_ip, "result": result}
        else:
            return {
                "success": False,
        ...
        return {"success": False, "test_ip": data.test_ip, "error": "GeoIP API test failed"}
...
def _validate_geoip_api_url(value: str) -> str:
    ...
    if literal_ip is not None and not literal_ip.is_global:
        raise ValueError("API URL must use a public host")
    try:
        parsed.port  # noqa: B018 - property access raises ValueError for invalid ports
    except ValueError as exc:
...
def create_custom_api(data: CustomApiConfig, _: bool = Depends(verify_session)):
    ...
    def add_custom_api_config(config: dict) -> dict:
        ...
        if any(
            requested_url_key is not None and _geoip_url_key(str(api.get("url") or "")) == requested_url_key
            for api in custom_apis
            if isinstance(api, dict)
        ):
            raise HTTPException(status_code=409, detail="GeoIP API URL already exists")
        known_ids = {api.get("id") for api in custom_apis if isinstance(api, dict)}
...
def update_custom_api(api_id: str, data: CustomApiConfig, _: bool = Depends(verify_session)):
    ...
    def update_custom_api_config(config: dict) -> dict:
        ...
        requested_url_key = _geoip_url_key(data.url)
        if any(
            api.get("id") != api_id and str(api.get("name") or "").strip().casefold() == data.name.casefold()
            for api in custom_apis
            if isinstance(api, dict)
        ):
            raise HTTPException(status_code=409, detail="GeoIP API name already exists")
