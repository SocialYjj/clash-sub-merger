...
class ServerModularityTests(unittest.TestCase):
    ...
    def test_server_uses_canonical_config_database(self):
        ...
        imports = [
            node for node in self.tree.body if isinstance(node, ast.ImportFrom) and node.module == "core.database"
        ]
    ...
    def test_server_does_not_keep_duplicate_app_config_class(self):
        class_names = {node.name for node in self.tree.body if isinstance(node, ast.ClassDef)}
