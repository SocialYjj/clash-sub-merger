...
def _parse_websocket_opts(get_param) -> dict:
    ...
    early_data_header = get_param("eh", "earlyDataHeaderName", "early-data-header-name")
...
def parse_vless_link(link: str) -> Optional[dict]:
    ...
    try:
        ...
        proxy = {"name": name, "type": "vless", "server": server, "port": int(port), "uuid": uuid_part, "udp": True}
        ...
        if transport in ["ws", "httpupgrade"]:
            ...
            path = get_param("path")
            host = get_param("host")
            _set_xhttp_opts(proxy, mode=xhttp_mode, path=path, host=host, extra=get_param("extra"))
...
def parse_vmess_link(link: str) -> Optional[dict]:
    ...
    try:
        ...
        proxy = {"name": name, "type": "vmess", "server": server, "port": int(port), "uuid": uuid_part, "udp": True}
            ...
            if plugin_value:
                ...
                if parts:
                    ...
                    if plugin_name in ["obfs-local", "obfs"]:
                        ...
                        if host:
                            plugin_opts["host"] = (
                                host.replace("obfs-host=", "") if host.startswith("obfs-host=") else host
                            )
                        if plugin_opts:
...
def parse_trojan_link(link: str) -> Optional[dict]:
    ...
    try:
        ...
        proxy = {"name": name, "type": "trojan", "server": server, "port": int(port), "password": password, "udp": True}
...
def parse_hysteria2_link(link: str) -> Optional[dict]:
    ...
    try:
        ...
        proxy = {"name": name, "type": "hysteria2", "server": server, "port": int(port), "password": auth}
...
def parse_tuic_link(link: str) -> Optional[dict]:
    ...
    try:
        ...
        proxy = {"name": name, "type": "tuic", "server": server, "port": int(port), "uuid": uuid, "password": password}
        _copy_uri_security(proxy, get_param)
...
def parse_anytls_link(link: str) -> Optional[dict]:
    ...
    try:
        ...
        proxy = {"name": name, "type": "anytls", "server": server, "port": int(port), "password": password}
        security = _copy_uri_security(proxy, get_param) or "tls"
...
def parse_wireguard_link(link: str) -> Optional[dict]:
    ...
    try:
        ...
        proxy = {"name": name, "type": "wireguard", "server": server, "port": int(port)}
...
def parse_hysteria_link(link: str) -> Optional[dict]:
    ...
    try:
        ...
        proxy = {"name": name, "type": "hysteria", "server": server, "port": int(port)}
...
def parse_socks_link(link: str) -> Optional[dict]:
    ...
    try:
        ...
        if scheme == "socks" and not parsed.username and not parsed.password and parsed.port is None and parsed.netloc:
        ...
        proxy = {"name": name, "type": "socks5", "server": server, "port": int(port)}
...
def parse_http_link(link: str) -> Optional[dict]:
    ...
    try:
        ...
        proxy = {"name": name, "type": "http", "server": server, "port": int(port)}
