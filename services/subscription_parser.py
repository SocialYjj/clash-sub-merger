...
def try_decode_base64(content: str) -> Optional[str]:
    ...
    try:
        ...
        raise Base64DecodeError(f"Base64 decoded content is not valid UTF-8: {e}") from e
...
def proxies_to_yaml(proxies: List[dict]) -> str:
    ...
    Returns:
        YAML string
    """
    return yaml.dump({"proxies": proxies}, allow_unicode=True, sort_keys=False, Dumper=YAMLDumper)
...
def parse_subscription_content(content: str) -> str:
    ...
    if not content:
        raise InvalidContentError("Empty content") from None
    ...
    raise InvalidContentError("Unable to parse content as YAML, Base64, or URI list") from None
...
def parse_local_subscription(content: str) -> Tuple[str, List[dict], int]:
    ...
    if proxies is not None:
        ...
        if valid_count <= 0:
            raise InvalidContentError("Subscription contains no valid proxy nodes") from None
