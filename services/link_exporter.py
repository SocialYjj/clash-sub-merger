...
def _conflicting_alias_reason(proxy: dict) -> str | None:
    ...
    pin_values = [
        str(proxy[key]) for key in ("fingerprint", "cert-sha", "ca-sha256") if proxy.get(key) not in (None, "")
    ]
...
def _wireguard_addresses(proxy: dict) -> list[str] | None:
    ...
    if legacy_address not in (None, "", []):
        legacy_values = (
            list(legacy_address) if isinstance(legacy_address, (list, tuple)) else str(legacy_address).split(",")
        )
...
def export_proxy_link(proxy: dict) -> LinkExportResult:
    ...
    if structural_reason == "unsupported-reality-option":
        ...
        if not (isinstance(reality_opts, dict) and set(reality_opts) <= {"public-key", "short-id", "spider-x"}):
...
def _proxy_to_link(proxy: dict) -> str:
    ...
    def append_transport_params(params: list) -> None:
        ...
        if network in ["ws", "httpupgrade"]:
            ...
            http_opts = (
                proxy.get("h2-opts", {}) if network == "h2" else proxy.get("http-opts") or proxy.get("h2-opts", {})
            )
            ...
            if extra_value:
                params.append(f"extra={q(json.dumps(extra_value, ensure_ascii=False, separators=(',', ':')))}")
        elif network == "kcp":
    ...
    try:
        if proxy_type == "vmess":
            ...
            if plugin:
                ...
                if plugin in ["obfs", "obfs-local", "simple-obfs"]:
                    ...
                    if plugin_opts.get("path"):
                        escaped_path = (
                            str(plugin_opts["path"]).replace("\\", "\\\\").replace("=", "\\=").replace(",", "\\,")
                        )
            ...
            userinfo = (
                base64.urlsafe_b64encode(f"{proxy.get('username', '')}:{proxy.get('password', '')}".encode())
                .decode()
                .rstrip("=")
            )
