...
class ProxyFilter:
    ...
    INFO_PREFIX_RE = re.compile(
        r"^\s*(?:建议|通知|公告|提示|说明|使用前|更新订阅|套餐到期|剩余流量)\s*[:：]?", re.IGNORECASE
    )
    ...
    STRONG_NODE_HINTS = ["节点", "备用", "家宽", "专线", "中转", "落地", "倍率", "游戏", "住宅", "原生"]
    ...
    def get_invalid_reason(proxy: dict) -> Optional[str]:
        ...
        if "官网" in name and ProxyFilter._OFFICIAL_URL_RE.search(name) and not ProxyFilter._has_node_identity(name):
    ...
    def get_clash_incompatible_reason(proxy: dict) -> Optional[str]:
        ...
        if normalized.get("_v2rayn-certificate-pin") not in (None, "") and normalized.get("fingerprint") in (None, ""):
    ...
    def _valid_port_ranges(value) -> bool:
        ...
        return (
            bool(segments)
            and len(segments) <= 28
            and all(ProxyFilter._valid_unsigned_range(segment, maximum=65535) for segment in segments)
        )
    ...
    def get_structural_invalid_reason(
        ...
        *,
        ...
        if (
            strict_clash
            and proxy_type == "trojan"
            and str(proxy.get("header-type", "") or "").strip().lower() == "http"
        ):
        ...
        if proxy_type in {"vmess", "vless", "trojan", "anytls"}:
            ...
            tls_enabled = bool(proxy.get("tls")) or bool(reality_opts) or proxy_type in {"trojan", "anytls"}
            tls_only_fields = (
                "sni",
                "servername",
                "alpn",
            ...
            if not tls_enabled and any(proxy.get(key) not in (None, "", False, [], {}) for key in tls_only_fields):
        ...
        if proxy_type == "hysteria2":
            ...
            if strict_clash and any(
                proxy.get(key) not in (None, "", [], {}) for key in ("ech", "pqv", "vcn", "fm", "finalmask")
            ):
