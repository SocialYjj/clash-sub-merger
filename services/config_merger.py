...
class ProxyGroupGenerator:
    ...
    def generate_groups(proxies: List[dict], country_groups: Dict[str, List[str]]) -> List[dict]:
        ...
        sorted_countries = sorted(
            [c for c in country_groups.keys() if country_groups[c]], key=lambda c: len(country_groups[c]), reverse=True
        )
        ...
        # 2. Manual Select
        node_select = {"name": "🚀 手动选择", "type": "select", "proxies": ["DIRECT", "REJECT"] + sorted_countries}
        ...
        for country in sorted_countries:
            if country in country_groups and country_groups[country]:
                country_group = {"name": country, "type": "select", "proxies": country_groups[country]}
...
"""
    ...
    def __init__(
        ...
        file_aliases: Dict[str, str] = None,
        ...
        for file_name in files:
            ...
            for proxy_index, proxy in enumerate(proxies):
                ...
                if self.output_format in {
                    ...
                    continue
                identified_proxy = strip_visibility_fields(ProxyFilter.sanitize_proxy(dict(proxy)))
                identified_proxy["_allocation_id"] = (
                    custom_node_id(proxy) if source_id == "custom_nodes" else unique_ids[proxy_index]
                )
    ...
    def merge_and_generate(self) -> dict:
        ...
        return {"proxies": self.all_proxies, "proxy-groups": proxy_groups}
