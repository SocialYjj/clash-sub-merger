...
from core.database import find_subscription_by_id, load_config
...
def find_custom_node(
    ...
    if node_id:
        for node in custom_nodes:
            if custom_node_id(node) != node_id or not is_node_enabled(node) or not ProxyFilter.is_valid_proxy(node):
                continue
